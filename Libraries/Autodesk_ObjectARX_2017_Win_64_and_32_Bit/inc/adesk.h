...
#pragma pack (push, 8)
    ...
    // compiler.
    //
    //We don't want to use #ifdefs throughout the code. Start weaning us off by
    //not defining any symbol for the WINRT build.
...
#define ADESK_UNREFED_PARAM(x) static_assert(sizeof(x)>0, "")
#else
#error Unknown compiler.
#endif

#if defined(_MSC_VER)
#define ADESK_THREAD_LOCAL thread_local
#elif defined(__clang__)
#if __has_feature(cxx_thread_local)
#define ADESK_THREAD_LOCAL thread_local
#else
#define ADESK_THREAD_LOCAL __thread
#endif
#else
#error Unknown compiler.
#endif
