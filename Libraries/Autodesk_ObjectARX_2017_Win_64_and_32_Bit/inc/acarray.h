...
// Caution: Usually deletes the existing memory buffer
// Also allocates a new buffer unless you set size to zero.
// Be careful about excessive heap usage.
//
template <class T, class R> AcArray<T,R>&
AcArray<T,R>::setPhysicalLength(int n)
{
    ...
    if (mPhysicalLen != 0) {
        // Allocate the physical memory buffer
