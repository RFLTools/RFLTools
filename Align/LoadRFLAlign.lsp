...
(defun C:RAMP (/ *error* ACCEPTRAMP ALSAVE ANGBASE ANGDIR C CANCEL CANCELRAMP CLAYER
 ...
 (defun SETSINGLELANE ()
  (set_tile "LANEWIDTH" (rtos 4.8))
  (set_tile "INSIDESHOULDER" (rtos 1.0))
 ...
 (defun SETDUALLANE ()
  (set_tile "LANEWIDTH" (rtos 7.4))
  (set_tile "INSIDESHOULDER" (rtos 2.0))
  (if (= "1" (get_tile "EXITTERMINAL"))
   (SETEXIT)
   (SETENTRANCE)
  )
 ...
 (defun SETEXIT ()
  (set_tile "NOSERADIUS" (rtos 1.1))
  (set_tile "TAPEROFFSET" (rtos 0.5))
  (if (= "1" (get_tile "SINGLELANE"))
   (progn
    (set_tile "GOREWIDTH" (rtos 5.5))
    (set_tile "WEAVEOFFSET" (rtos 8.5))  ;  Updated to Rev.3 (old = 8.2)
   )
   (progn
    (set_tile "GOREWIDTH" (rtos 8.1))
    (set_tile "WEAVEOFFSET" (rtos 11.1))  ;  Updated to Rev.3 (old = 10.8)
   )
  )
 )

 (defun SETENTRANCE ()
  (set_tile "NOSERADIUS" (rtos 0.6))
  (set_tile "TAPEROFFSET" (rtos 0.0))
  (if (= "1" (get_tile "SINGLELANE"))
   (progn
    (set_tile "GOREWIDTH" (rtos 5.3))
    (set_tile "WEAVEOFFSET" (rtos 5.3))  ;  Updated to Rev.3 (old = 5.75)
   )
   (progn
    (set_tile "GOREWIDTH" (rtos 7.9))
    (set_tile "WEAVEOFFSET" (rtos 7.9))  ;  Updated to Rev.3 (old = 8.35)
   )
  )
 )
...
(defun RFL:RAMPDRAW (ENTNOSE MODE / ALSAVE ANG ANG2 ANGBASE ANGDIR C CMDECHO D1 D2 DIMZIN DOFFSET DSTA ENTDELSET ENTLISTNEW ENTNOSELIST
                                    ...
                                    ORTHOMODE OSMODE RLSAVE P1 P2 PL1 PL2
...
;       (entmake ENTLISTNEW)
       (command "._PLINE")
       (while (<= C (cdr (assoc "RAMPVERTICIES" RFL:RAMPLIST)))
        (command (RFL:XY (list (+ STA1 (* C DSTA))
                               (+ OFFSET1 (* C DOFFSET)))))
        (setq C (+ C 1))
       )
       (command "")
...
(defun RFL:CALCROUNDABOUT (ENT NMAX / ANG ELEV ENTLIST N SLOPE)
...
(defun C:TPROFROUNDABOUT (/ *error* ANGBASE ANGDIR CMDECHO INC ORTHOMODE OSMODE STA STA1 STA2 Z)
 (command "._UNDO" "M")
 (defun *error* (msg)
  (setvar "CMDECHO" CMDECHO)
  (setvar "ANGBASE" ANGBASE)
 ...
 (if RFL:ALIGNLIST
  (progn
   ...
   (setq STA1 nil)
   (while (= nil STA1)
    (if (setq STA1 (getpoint "\nStarting point (<return> to enter station) : "))
     (setq STA1 (car (RFL:STAOFF STA1)))
     (setq STA1 (getreal "\nEnter start chainage : "))
    )
   )
   (setq STA2 nil)
   (while (= nil STA2)
    (if (setq STA2 (getpoint "\nEnding point (<return> to enter station) : "))
     (setq STA2 (car (RFL:STAOFF STA2)))
     (setq STA2 (getreal "\nEnter end chainage : "))
    )
   )
   (setq INC 0.0)
   ...
   (if (< STA2 STA1) (RFL:SWAP STA1 STA2))
