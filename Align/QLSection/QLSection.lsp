...
(defun C:CREATELIDARDB (/ DBSOURCE DBFILE)
 (if (setq DBSOURCE (getfiled "Select a source point file" "" "*" 2))
  (if (setq DBFILE (getfiled "Select a target for new .db file" "" "db" 1))
   (if (setq DELIM (getstring T "\nEnter delimiter : "))
    (RFL:CREATELIDARDB DBSOURCE DBFILE DELIM T)
    (princ "\nNo delimeter given!")
   )
   (princ "\nNo target given!")
  )
...
(defun RFL:CREATELIDARDB (DBSOURCE DBFILE DELIM NEWFLAG / C CHKERR CREATEDATABASE INFILE INLINE)
 (defun CHKERR (CMD)
  (if (not CMD)
   (progn
    (princ (DSQL_LASTERR))
   ...
   (if (or NEWFLAG
    ...
    (progn
     ...
     (while (setq INLINE (read-line INFILE))
      (if (and (/= (setq X (RFL:COLUMN INLINE 1 DELIM)) "")
               (/= (setq Y (RFL:COLUMN INLINE 2 DELIM)) "")
               (/= (setq Z (RFL:COLUMN INLINE 3 DELIM)) "")
	      )
       (progn
